...
export const AddDistrictPopUp = (props) => {
	...
	const onSave = async () => {
		...
		setLoading(true);
		const response = await dispatch(saveDistrict({ stateId, regionId, districtName, isActive }));
		setLoading(false);
		if (!response.error) {
		...
		<PopUp>
			<div className="pb-10 bg-white rounded-lg">
				<div className="flex bg-first p-4 text-white justify-between rounded-t-lg">
					<h1 className="text-l font-weight-[300]">Add District</h1>
