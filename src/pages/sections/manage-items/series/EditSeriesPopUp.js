...
import { getSeriesList, saveSeries } from "../../../../redux/apis";

export const EditSeriesPopUp = (props) => {
    ...
    useEffect(() => {
        if (props.isVisible) {
            setSeriesName(props.isVisible.seriesName);
    ...
    const onSave = async () => {
        if (seriesName === "" || seriesName === undefined || seriesName === null) {
            toast.error("Please enter series name");
            return;
        }
        setLoading(true);
        const seriesId = props.isVisible.seriesId;
        const response = await dispatch(saveSeries({ seriesId, seriesName, isActive }));
        setLoading(false);
        console.log("seriesName response", response);
        if (!response.error) {
            if (response.payload.isSuccess) {
                toast.success(response.payload.message);
                props.onCancel(); // Close PopUp
                dispatch(getSeriesList()); // Refresh series list
            } else {
        ...
        <PopUp>
            <div className="pb-10 bg-white rounded-lg">
                <div className="flex bg-first p-4 text-white justify-between rounded-t-lg">
                    <h1 className="text-xl font-weight-[300]">{isView ? "View Series" : "Edit Series"}</h1>
