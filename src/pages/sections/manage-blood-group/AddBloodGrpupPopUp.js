...
export const AddBloodGrpupPopUp = (props) => {
	...
	const onSave = async () => {
		if (bloodGroupName === '' || bloodGroupName === undefined || bloodGroupName === null) {
			toast.error("Please enter blood group name");
			return;
		}
		setLoading(true); 
		const response = await dispatch(saveBloodGroupDetails({ bloodGroupName, isActive }));
		setLoading(false);
		if (!response.error) {
		...
		<PopUp>
			<div className="pb-10 bg-white rounded-lg">
				<div className="flex bg-first p-2 text-white justify-between rounded-t-lg">
					<h1 className="text-l font-weight-[400]">Add Blood Group</h1>
